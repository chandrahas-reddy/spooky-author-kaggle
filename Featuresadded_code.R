...
spooky_topics<-tidy(lda, matrix="beta")
...
ggplot(rankImportance, aes(x = reorder(Variables, Importance), 
  ...
  geom_text(aes(x = Variables, y = 1, label = Rank),
            ...
            fontface = 'bold') +
  labs(x = 'Top Variables', title = 'XG Boost Model : Relative Variable Importance', 
       y = 'Importance Score') +
  coord_flip() + 
  theme_bw()
...
yXb_predict_prob <- predict(AuthorXGB, newdata = spookyLabeledTerms.Test, type = "prob")
...
#--------Part-5--------#
# Performing prediction on test data set provided by Kaggle
# 1. Read the dataset to a variable 'test'
# 2. Adding text length to test
# 3. Adding comma features to test
# 4. Adding Sentiment Analysis


# 1. Reading dataset to variable 'test'
...
# 4. Adding Sentiment analysis
spooky_sentiment.test <- as.data.frame(test$text)
colnames(spooky_sentiment.test)[1] <- "text"

spooky_sentiment.test  <- sapply(spooky_sentiment.test ,function(row) iconv(row, "latin1", "ASCII", sub=""))

#sentiment score
sentiment_score <- score.sentiment(spooky_sentiment.test, pos, neg, .progress='text')
summary(sentiment_score)
View(sentiment_score)
...
# 5. Preparing model to apply model

spookyLabeled <-funcDTM(test)

colnamesSame = intersect(colnames(spookyLabeledTerms),colnames(spookyLabeled))
 
spookyLabeled = spookyLabeled[ , (colnames(spookyLabeled) %in% colnamesSame)]

spookyLabeled$Length <- test$TextLength

spookyLabeled$sentimentscore <- test$Sentimentscore

spookyLabeled$Ncommas <- test$commas

spookyLabeled$Nsemicolons <- test$semicolumns

spookyLabeled$Ncolons <- test$colons

# predicting the author value from the Test data
yXb_test_predict <- predict(AuthorXGB, spookyLabeled)
yXb_test_predict_prob <- predict(AuthorXGB, spookyLabeled, type = 'prob')

# Save the solution to a dataframe
solution <- data.frame('id' = test$id, yXb_test_predict)
solution <- data.frame('id' = test$id, yXb_test_predict_prob)
